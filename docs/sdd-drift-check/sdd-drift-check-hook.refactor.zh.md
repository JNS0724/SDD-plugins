...
本计划中的 P0.5 / P1 / P2 / P3 / P4 已进入落地状态：

- 开发态已拆为 `src/adapters/`、`src/core/`、`src/handlers/`、`dispatcher.js`。
- 分发态仍提交两个单文件产物：
  - `plugins/sdd-drift-check/sdd-drift-check-hook.js`
  - `plugins/sdd-drift-check/sdd-drift-check-opencode.js`
- 当前主用户面为 Claude Code command hook 与 OpenCode native plugin。
- OMO 桥接文档仅作为历史兼容参考，不再作为主验收目标。
- Prompt 文案已抽到 `src/core/prompts.js`，统一输出
  `<system-reminder>` + `[SYSTEM DIRECTIVE: SDD-DRIFT-CHECK - <TYPE>]`
  结构。

后续重构不应重新把业务规则搬回 adapter；adapter 只负责运行时输入/输出协议，SDD drift 判定、prompt、report、state storage 继续收敛在 `src/core/`。
...
- **OpenCode / 旧 OMO 版本升级**：主路径锁定 `opencode-ai@1.2.27` / `@opencode-ai/plugin@1.2.27`；`oh-my-opencode@3.17.2` 仅为历史桥接参考
...
- 继续 P1 骨架：新增 `src/actions.js` 与 `src/dispatcher.js`，导出 `Actions`、`runActions`、`HookHandlers`、`createHookHandlers`；新增单测验证 Action 执行顺序为 `LOG → SAVE_PROJECT → SAVE_SESSION → REFRESH_REPORT → EMIT_MESSAGE`，并验证 5 个 hook 入口均已注册。
- 完成 P1 handler 第一轮物理抽离：新增 `src/handlers/{user-prompt-submit,pre-compact,pre-tool-use,stop,post-tool-use}.js`，并把 PostToolUse 无文件 checkpoint 分支拆到 `post-tool-use-checkpoint.js`；`main()` 缩到只负责 stdin parse + `dispatch(input)`，全部 handler 文件均不超过 200 行。
- 完成 P2 CircuitBreaker 接线：新增 `CircuitBreaker.{isOpen,recordFailure,recordSuccess}`，支持 `SDD_DRIFT_CIRCUIT_MAX_FAILURES` / `SDD_DRIFT_CIRCUIT_COOLDOWN_MS`；`dispatch()` 捕获 handler 异常并记录 `handler_exception`、`circuit_open`、`circuit_open_skip`、`circuit_close` 诊断事件，单测覆盖 5 次失败后短暂静默。
- 开始 P3 Attribution：新增 `src/attribution.js`，落地 `Attribution.decide()` 的 `no-attribution` / `single` / `session-touched` / `active-ttl` / `needs-review` 决策树，并接回 `collectProjectAttributionTargets()`；单测覆盖 session touched、active TTL path similarity、needs-review 和 no-attribution。
- 补充 P3 J13 旅程 fixture：验证多 change-dir 下 active TTL + 路径相似只把 `src/alpha/*` 归因到 alpha；同时修复 session 级宽泛 code gap 与 project 级归因结果重复/冲突的问题，project 已跟踪的 code file 由 project gap 作为权威来源。
- 完成 P3 needs-review 评审 fork：模糊归因时不再把代码同时挂到所有候选 change-dir，而是记录 `state.attributionReviews[signature]` 并注入 `buildAttributionReviewPrompt()`；后续 SDD edit/read 或 Stop 兜底会标记 `edit` / `no-edit-confirmed` / `unrelated` resolution。
- 补充 P3 J11 旅程 fixture：验证 unrelated code 在多活跃 change-dir 下只触发一次归因评审提示，不污染 project linkedCode，第二次 Stop 兜底接受为 unrelated 并留下人工可查报告。
- 完成 P4 schema 收尾：ProjectState 层 `peerSyncs` 改名为 `docSyncs`，旧 `project.json` 透明迁移，保存后不再持久化旧字段；SessionState 层 `peerSyncs` 保持不变。
- 补充 P4 R8 诊断采样：`handler_exception`、`hook_exception`、`circuit_open`、`circuit_open_skip` 按窗口汇总为 `diagnostic_summary`，避免错误风暴时只能逐行排查。
- 设计文档升至 v1.2：同步 `docSyncs` schema、R8 采样规则，以及 Dispatcher / CircuitBreaker / Attribution 已实施状态。

### 2026-05-26：Prompt 结构与双运行时回归

- `src/core/prompts.js` 统一生成 `<system-reminder>` 包裹的结构化模型可见提示词。
- 每类提醒使用 `[SYSTEM DIRECTIVE: SDD-DRIFT-CHECK - <TYPE>]` 标识，包括 code review、peer sync、question checkpoint、Stop enforcement、compaction recovery、carry-over drift 和 attribution review。
- Prompt 测试补齐结构锚点，避免后续改文案时丢掉模板保护、原任务恢复和 no-op edit 禁止规则。
- 使用 DeepSeek 与 MiniMax 分别回归 OpenCode native plugin 与 Claude Code command hook 的重点场景，确认无未处理 `.sdd-drift-report.md`。
