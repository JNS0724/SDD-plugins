...
**版本:** 1.2
**日期:** 2026-05-22
**修订记录:** v1.2 — 修正 peer 文件不存在时不触发 doc-doc drift；明确 `codeAheadOfDocs` 逐文档 review；新增 `PreToolUse` 提问/交接 checkpoint 范围；统一 ProjectState 存储路径为 `<stateDir(cwd)>/project.json`
...
OMO 3.17.2 + CC 共有面：PostToolUse / Stop / UserPromptSubmit / PreCompact。`PreToolUse` 作为可选 question checkpoint：仅用于提问/交接类工具；若桥接不支持则自动降级到 Stop / UserPromptSubmit。
...
- 不支持该 hook 的 OpenCode / OMO 路径，仍由 Stop 和 UserPromptSubmit 兜底
