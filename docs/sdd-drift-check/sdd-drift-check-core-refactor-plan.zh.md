...
截至 2026-05-26，目标结构已经从“规划态”推进到“可运行态”：

```text
plugins/sdd-drift-check/src/
├─ adapters/
│  ├─ claude-code/command-hook.js
│  └─ opencode/native-plugin.js
├─ core/
│  ├─ attribution.js
│  ├─ diagnostics.js
│  ├─ drift-engine.js
...
- `src/attribution.js` 是旧兼容导出，后续可在确认无外部调用后归并到
  `src/core/attribution.js`。
- 仍需持续保证 adapter 只做运行时输入/输出归一化，业务规则继续留在
  `src/core/`。
- 新增 prompt 或输出协议时，必须先补 `test-core-prompts.cjs` 这类
  characterization test，再构建发布件。
...
```text
<system-reminder>
[SYSTEM DIRECTIVE: SDD-DRIFT-CHECK -
...
PEER SYNC CHECKPOINT
PEER SYNC REMINDER
QUESTION CHECKPOINT
STOP ENFORCEMENT
COMPACTION CHECKPOINT RECOVERY
COMPACTION DRIFT SUMMARY
CARRY-OVER DRIFT
ATTRIBUTION REVIEW
```
