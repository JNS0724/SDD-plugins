...
- **v0.3**（本版）：响应 2026-05-29 Codex 评审（§15）——5 点 P0/P1 收敛为 2 个根因修复 + 1 个待拍板决策。逐点响应见 §16。
...
{ "id": "ev_<hash>", "ts": "...", "session": "sess_A", "kind": "align",
  "link": "greeting-behavior", "codeHash": "sha256:a1b2c3", "sddHash": "sha256:d4e5f6",
  "declaredBy": "agent", "source": "stop-confirm", "reason": "实现完成，确认对齐" }
...
- **事件 id 内容寻址**（`hash(payload)` 或 UUID），不用全局递增 `seq`——天然幂等去重，跨进程/分支无冲突（解 Codex P0-2 / P1-2，详见 §8.1、§16）。
- **顺序不存在事件里**：`deriveLock()` 重放时用 `{ts, session, id}` 计算偏序。
  ...
  "links": [
    {
      ...
      "alignedHash": null             // 确认对齐那刻的 code/design/tasks hash 快照 | null
    }
  ],
  "unresolved": [
...
### 6.5 ★ 待拍板决策：happy-path 的对齐确认策略

Codex P0-1 修复后（auto-coedit 只建归属、`aligned:false`），引出一个**必须由产品拍板**的张力：

> J1（同会话写完 design+tasks+code）结束时，link 是 `aligned:false`。若 `sdd check` 把它当漂移报出来，就**每次实现都打扰**，违反"happy path 不烦"。

解法是把 `computeDrift` 输出从二值拆成**多态**（§7），让 `aligned:false` 落在 **`UNCONFIRMED`（advisory，永不 block）** 而非 `DRIFT`。在此之上，终态策略二选一：

| 选项 | 行为 | 优点 | 代价 |
|---|---|---|---|
| **A. UNCONFIRMED 可作终态** | auto-coedit link 永久 advisory，直到有人 `align`；不强制 | happy path 零额外步骤 | 仓库长期存在"未确认对齐"的 advisory |
| **B. 实现结束须 align 一次** | Stop / 收尾时 agent 做一次 `align` 决策事件（按批，非按文件）覆盖本轮 | 基线确认是显式事实，最干净 | 每次实现多一步确认 |

**我的推荐：A 为默认 + B 可配置（`sdd check --require-aligned`）**。理由：
- `UNCONFIRMED` 是**诚实状态**（"对齐未经确认"本就是真的），advisory 不阻断——不算"误报漂移"，满足原始约束。
- 工具**本就无法验证语义对齐**，只能追踪"有没有人显式声明过 + 声明后内容有没有变"。强制 align 也只是"agent 声称对齐"，未必更真。

**待你拍板：** 默认走 A 还是 B？这决定 `sdd check` 默认退出码语义与 happy-path 体验。

---

## 七、校验层（drift 纯函数）

**多态输出（v0.3 核心修正，解 Codex P0-1）**：不再二值"漂移/不漂移"，而是按 link 的 `aligned` 与内容哈希分态：

```
computeDrift(工作树, sdd.lock) → DriftReport:    # 纯函数，无副作用
  for link in lock.links:
    if link.aligned == false:
        → UNCONFIRMED        # 已归属、从未确认对齐（含 auto-coedit）。advisory，永不 block
    else:  # aligned == true，曾被 align 决策事件确认
        if hash(当前 code) ≠ link.alignedHash.code        → CODE_DRIFT   # 确认后代码又变 → 真漂移，block 级
        elif hash(当前 design 段) ≠ link.alignedHash.design → DOC_DRIFT
        elif hash(当前 tasks 项) ≠ link.alignedHash.tasks   → DOC_DOC_DRIFT
        else → ALIGNED         # 静默
  + lock.unresolved 非空 → UNRESOLVED        # 无归属。advisory / block 按 §6.5 策略
  + 改动过、不属于任何 link/unresolved/impactDecision 的 code → UNLINKED
  + 指向已删除文件/段落的 link → ORPHAN

退出码：CODE/DOC/DOC_DOC_DRIFT → 非零（block）；UNCONFIRMED/UNRESOLVED → 默认 0（advisory），
        `--require-aligned`（§6.5 选项 B）下 UNCONFIRMED 也非零。
```

**关键区分：`UNCONFIRMED`（从未确认对齐）≠ `DRIFT`（确认后又变）。** auto-coedit 落 UNCONFIRMED（advisory，不算误报漂移），真 block 级 DRIFT 只发生在**曾经 align 确认过**的 link 内容又变——这才无歧义。
...
deriveLock(events, 工作树) → sdd.lock:
  1. 排序：按 {ts, session, id} 计算偏序（id 内容寻址，不依赖全局 seq）
  2. 去重：按 event id 去重（幂等）
  3. 回放：
     code-edit / doc-edit（事实）→ 更新元素哈希、跑确定性解析建**候选归属**（aligned: false）
     align（决策）            → 置对应 link aligned: true + alignedBy + alignedHash 快照
...
- **只有 align 决策事件能设对齐基线**：事实事件（含 auto-coedit）永远只建归属、`aligned:false`（解 Codex P0-1）。语义正确性靠 code review，不是本工具职责。
...
8. **happy-path 终态策略未定**：§6.5 待拍板（A UNCONFIRMED 可作终态 / B 强制 align-once）。
...
或先拍板 §6.5（happy-path 终态策略），再就薄弱点（**link 粒度** / **事件日志 compaction**）定方案后写代码。
