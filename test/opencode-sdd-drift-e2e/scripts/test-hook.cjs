...
try {
  {
    ...
    assert.match(enforcement, /保留它已有的 Markdown 模板/)
    assert.match(enforcement, /保持所有已有标题行不变/)
    assert.match(enforcement, /不要把整篇文档替换成/)
    assert.match(enforcement, /不要为了满足插件提醒而新增章节/)
    assert.match(enforcement, /不要删除无关的已有段落/)
    assert.match(enforcement, /找到最合适的已有标题、段落、列表项或任务项/)
    assert.match(enforcement, /从暂停处继续原始用户请求/)
    const compact = hook.buildToolEnforcement(gaps, { compact: true })
    assert.match(compact, /SDD drift reminder/)
    assert.match(compact, /从暂停处继续原始用户请求/)
...

  {
    ...
    assert.match(enforcement, /活跃 SDD 文档都是实时计划记录/)
    assert.match(enforcement, /不要把它当成“无需更新文档”/)
    assert.match(enforcement, /不要只添加标记、完成说明或泛泛摘要/)
    assert.match(enforcement, /文档描述真实实现的行为、API、错误处理、性能策略或任务状态/)
    assert.match(enforcement, /字面量返回值/)
    assert.match(enforcement, /没有旧表述仍然与当前代码矛盾/)
    assert.match(enforcement, /改变了行为、API 或契约、算法、状态或数据流/)
    assert.match(enforcement, /当代码完成、变更、取消、拆分/)
    assert.match(enforcement, /只有纯机械改动才适合走“无需改文档”路径/)
    assert.match(enforcement, /明确说明已评审哪些活跃 design\.md\/tasks\.md/)
    assert.match(enforcement, /只读评审 subagent/)
    assert.match(enforcement, /保留它已有的 Markdown 模板/)
    assert.match(enforcement, /保持所有已有标题行不变/)
    assert.match(enforcement, /不要把整篇文档替换成/)
    assert.match(enforcement, /不要为了满足插件提醒而新增章节/)
...

  {
    ...
    assert.match(pending.message, /只读评审 subagent/)
    assert.match(pending.message, /从暂停处继续原始用户请求/)
...

  {
    ...
    assert.match(compactSummary, /从暂停处继续原始用户请求/)
...

  {
    ...
    assert.match(prompt, /判断 SDD 文档是否需要修改/)
    assert.match(prompt, /纯机械改动/)
