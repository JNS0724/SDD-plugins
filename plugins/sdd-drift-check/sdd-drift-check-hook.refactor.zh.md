...
```json
{
  ...
  "scripts": {
    ...
    "build:check": "node build.mjs && git diff --exit-code sdd-drift-check-hook.js"
  },
  "devDependencies": {
    "esbuild": "^0.21.0"
  }
...
> 注意 `package.json` 用 `"type": "module"` 是为了让 `build.mjs` 直接跑；hook 产物本身仍是 CommonJS（由 esbuild `format: "cjs"` 强制）。
...
- name: Verify dist artifact matches src
  ...
  run: |
    npm ci
    npm run build
    git diff --exit-code sdd-drift-check-hook.js
```

`exit-code` 非 0 即意味着有人改了 `src/` 没跑 `npm run build`，或直接改了产物文件——两种情况都拒绝合入。
