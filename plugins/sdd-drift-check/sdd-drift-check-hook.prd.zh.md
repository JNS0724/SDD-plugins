...
**版本:** 1.1
**日期:** 2026-05-22
**修订记录:** v1.1 — 新增 J9–J16 旅程；FR11 改为 LLM 评审驱动归属；新增 FR12 / FR13；新增附录 A `ATTRIBUTION_REVIEW_RULES` 草案
...
**版本锁定:** `oh-my-opencode@3.17.2` + `opencode-ai@1.2.27`
...
不解决：自动 review（auto-review）、PreToolUse 阻断（已排除）、LLM 语义理解。
...
| Stop 期望 | **提问前应提醒 drift**，或在 hook 输出中"提醒 agent 在向用户提问时给出 SDD 偏差检测选项" |
| 关键挑战 | 从 hook 视角无法可靠区分"提问的 Stop"与"终结的 Stop"。设计取舍：**Stop 一律执行 drift 检测**——agent 自行决定是否在问题前向用户报告 |
...
| J8 | 是 | 同 J6，但 Stop 是提问态 | 否 | **提醒**（同 J6） |
...
- 在 `.sdd-drift-hook-state/project.json` 维护**跨会话**的 change-dir 状态
...
- OMO 端利用 UserPromptSubmit 的 `parentSessionId` 进一步加固
...
| NFR3 | 支持 Claude Code 原生 + OMO 3.17.2 桥接两条路径，行为一致；能力差异自动降级 |
...
#### ProjectState（`.sdd-drift-hook-state/project.json`）
...
#### SessionState（`.sdd-drift-hook-state/session/<hash>-<sessionID>.json`）
...
conditions = {
  ...
  designAheadOfTasks:  design.lastEditedMs > tasks.lastEditedMs
  tasksAheadOfDesign:  tasks.lastEditedMs > design.lastEditedMs
  codeAheadOfDocs:     max(linkedCode.lastEditedMs)
                       > max(design.lastReviewedMs, tasks.lastReviewedMs)
...
```
codeAheadOfDocs(dir):
  return max_or_zero(dir.linkedCode.lastEditedMs)
       > max_or_zero(dir.docs.design.lastReviewedMs,
                     dir.docs.tasks.lastReviewedMs)
       && max_or_zero(dir.linkedCode.lastEditedMs) > dir.alignedAtMs
```
...
OMO 3.17.2 + CC 共有面：PostToolUse / Stop / UserPromptSubmit / PreCompact。
...
- OMO 3.17.2 源码确认：error / interrupt session 下 Stop block 被丢
- Hook 始终先 `refreshReport` 写 `.sdd-drift-report.md`，保证 block 失效时用户仍可见
...
| PreToolUse 阻断 | 已排除（用户在前期决策中明确不需要）|
...
| FR5 子代理归属 | mtime + 文本启发式（[L1593-1791](./sdd-drift-check-hook.js)） | **加固**：OMO 端用 parentSessionId 增强；现有启发式保留兜底 |
| FR6 跨会话 carry-over 提醒 | 无 | **缺**：需 UserPromptSubmit handler |
| FR7 Stop 强制检测 | 当前 Stop 已检测，但无 J1/J2 例外 | **细化**：与 FR2 / FR3 联动 |
...
| NFR3 CC + OMO 兼容 | 已两端支持 | **加固**：双向能力探测 |
...
5. **阶段 4**：实现 FR5 加固（OMO 端 parentSessionId）+ FR6（UserPromptSubmit 注入）
