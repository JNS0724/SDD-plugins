...
## 10. 2026-06-01 更新：默认收敛为 `once` 模式

依据 2026-06-01 真实体验报告（`sdd-review-ledger-ux-report-2026-06-01.zh.md` §6）：改进一把单阶段提醒从 7/5
次降到 2/3 次，但「路径集一增长就再提醒」对普通开发任务仍偏密。报告建议默认从「增长就提醒」收敛为
「每个用户 turn 只主动提醒一次，结束点聚合兜底」。已落地为一个**提醒模式开关**：

```text
SDD_REVIEW_REMINDER_MODE=once    # 默认：体验优先
SDD_REVIEW_REMINDER_MODE=growth  # opt-in：安全/审计优先
```

- **`once`（默认）**：同一 turn 至多一条主动提醒。首次相关变更 → 完整提醒；之后即使 pending 路径集增长，
  也**只更新 ledger/todo、不再主动注入**。兜底三件套不变：始终重写的 `.sdd-review-todo.md`、改进三的
  Stop/idle 收尾扫、下一轮 prompt 的 carry-over。即「先让 agent 把这一批改完，再统一评审」。
- **`growth`（opt-in）**：保持 §2/§3 行为——路径集增长就再提醒（第二条起为精简体）。适合强审计场景。

实现：`config.js` 读 `SDD_REVIEW_REMINDER_MODE`（未知值 → `once`）；`throttle.js` 的 `decideReminder`
据 `mode` 决定 `once` 抑制全部同回合重复 / `growth` 只抑制无增长重复；`on-edit.js` 透传 `cfg.reminderMode`。
不变量全部保留（清除只认勾选、fail-open、工具不答语义、模板字节稳定）。`once` 模式把「同回合新路径」交给
结束点兜底——CC 侧由 Stop 强制评审一次，OpenCode 侧靠 idle + 下轮 carry-over（仍是 §4.5 的诚实不对称）。

> 注：报告 §6.2「后续只发精简体」此前已由改进二落地；§6.1/§6.3（once 默认 + 模式开关）即本次更新。

## 11. 2026-06-01 更新（B）：文档领先代码 = 计划，不主动催、不自动写实现

**问题**：在"文档领先代码"时（只改了 `design.md`/`tasks.md`、code 还没跟上），旧设计会触发主动评审，
模型据 rules.md 的「反向…决定 code 要不要跟进」直接动手改代码。对用户而言这过于激进——改设计文档常常
是"在规划 / 表达意图"，不是"下达实现命令"。根因是把**检测到漂移**与**立即消除漂移（写代码）**合并了。

落地为 A+B 两层（**不引入新配置**，YAGNI）：

- **A（评审语义）**：`sdd-review-rules.md` 改写——design/tasks 领先 code 是**计划而非缺陷**，评审只确认
  文档本身、必要时记一句"code 尚未实现 X"，**不在评审里替用户补写实现代码**；实现是用户单独发起的步骤。
  第 4 步"需改"明确为"编辑 design/tasks 让**文档**与 code 现状一致"（同步文档，不是改实现）。
- **B（触发方向）**：主动通道（`on-edit` 提醒 / `on-stop` 收尾 block / `on-prompt` carry-over）只由
  **code 变更**驱动；doc 变更永远只被动写进 `.sdd-review-todo.md`，不主动打扰。新增纯函数
  `compute.js: selectActiveNeeds(items) = items.filter(kind === "code")`，三个 handler 据此过滤；todo
  写入仍是**全量** needs（在 `pipeline.run` 内，绝不丢项）。

**为什么不违反底线**：方向只看机械的 `item.kind`（即 `classifyPath` 结果），工具仍不下任何"已对齐 /
有偏差"的语义结论（§7「工具不答语义」）；doc 漂移仍被完整记录（§7「永不静默丢义务」）；清除只认勾选不变。
...
> 是长任务的最后防线；默认 `once` 模式让"每个 turn 一次提醒"成为开箱体验，`growth` 留给强审计；
> 改进 B 让"文档领先代码"被当成计划——不主动催、也不在评审里自动写实现。**
> 全部不碰「让工具答语义」「永不阻断用户主流程」「清除只认勾选」这几条底线。
