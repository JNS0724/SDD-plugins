...
**MVP 已实现（Claude Code 单平台）。** M1 核心纯函数 + M2 管线 + M3 Claude Code 适配器全部完成，121 个测试通过，发布件 `sdd-review-ledger-hook.js` 已打包并烟测可用。OpenCode 适配器（M4）、双平台 e2e（M5）后置。
...
npm run build      # 打包出 sdd-review-ledger-hook.js
npm run build:check  # 校验发布件与源码同步（字节级）
```

源码在 `src/`（CommonJS，无运行时依赖）：`src/core/` 纯逻辑可单测，`src/pipeline.js` 单次运行主干，`src/handlers/` + `src/dispatch.js` + `src/adapters/claude-code/` 平台适配。构建顺序与里程碑见详细设计 §15/§16。
